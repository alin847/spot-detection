...
class CellDetection:
    def __init__(self, gray_image):
        ...
        spot_detection(scales, threshold): detects spots in the image using
        the a trous wavelet transform.
            scales: the number of times the decomposition is performed (J).
            Choose larger scales for larger spots.
            threshold: the threshold multiplier to filter the last wavelet plane.
            Default is 4.5.
        localization(region_size, min_distance): localizes the cell locations
        using the radial symmetry method.
            region_size: the size of the square region to extract. This number
            should be slightly larger than the spot size. This number must be odd.
        view_locations(): returns the image with the cell locations overlaid
    ...
    def localization(self, region_size=11, min_distance=3):
        """
        ...
        method on a 11 by 11 grid (default).
        ...
        def extract_region(image, center, size):
            ...
            i, j = center
            region = np.zeros((size, size), dtype=image.dtype)
        ...
        for i, j in centroids:
            ...
            y, x = radial_symmetry(local_region)
            y += i - region_size // 2
            x += j - region_size // 2
            cell_locations.append([y, x])
    ...
    def view_locations(self):
        ...
        )  # auto adjust brightness not necessary
